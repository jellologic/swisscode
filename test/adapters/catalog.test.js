...
import {
  ...
  normalizeOpenRouter,
} from '../../src/adapters/catalog/openrouter.js'
import {
  MODELSCOPE_CAPABILITIES,
  createModelScopeCatalog,
  normalizeModelScope,
} from '../../src/adapters/catalog/modelscope.js'
import { createCatalogRegistry } from '../../src/adapters/catalog/registry.js'
import { createFsCacheStore } from '../../src/adapters/store/fs-cache-store.js'
import { CACHE_TTL_MS, CACHE_VERSION } from '../../src/core/catalog.js'
...
test('the ModelScope catalog endpoint is the /v1 OpenAI route, unlike the base URL', async () => {
  const { MODELSCOPE_ENDPOINT } = await import('../../src/adapters/catalog/modelscope.js')
  const { modelscope } = await import('../../src/adapters/providers/modelscope.js')
...
function harness({ payload = OPENROUTER_PAYLOAD, now = 1_000_000_000_000 } = {}) {
  ...
  return { dir, clock, calls, net, cache, setNow: (n) => { now = n } }
}
