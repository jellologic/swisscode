...
import {
  ...
  makeIsExecutable,
  spawnFallback,
} from '../../src/adapters/process/node-process.js'
...
function fakeHost() {
  ...
  host.exits = []
  host.kills = []
  host.exit = (code) => host.exits.push(code)
  host.kill = (pid, signal) => host.kills.push([pid, signal])
  return host
}

test('the spawn fallback relays a normal exit code', async () => {
  const host = fakeHost()
  spawnFallback(process.execPath, ['-e', 'process.exit(3)'], process.env, host)
...
test('a missing binary reports 127 rather than throwing', async () => {
  const host = fakeHost()
  spawnFallback('/nonexistent/claude-binary', [], process.env, host)
