...
import { PROVIDERS, REJECTED_PROVIDERS, byId } from '../src/adapters/providers/registry.js'
import { CATALOG_IDS } from '../src/adapters/catalog/registry.js'
import { SUFFIX } from '../src/core/context.js'
import { buildEnvPlan, COMPAT_ENV } from '../src/core/env.js'
import { TIERS, TIER_ENV_VARS } from '../src/core/tiers.js'
...
for (const p of PROVIDERS) {
  ...
  test(`descriptor ${p.id}: defaultModels only names real tiers`, () => {
    for (const tier of Object.keys(p.defaultModels)) {
      assert.ok(TIERS.includes(tier), `${p.id} declares unknown tier "${tier}"`)
    }
  })
  ...
  test(`descriptor ${p.id}: every tier gets [1m] or none does`, () => {
    ...
    const plan = buildEnvPlan({ provider: p.id, apiKey: 'k' }, p, {})
    const values = TIER_ENV_VARS.map((v) => plan.set[v]).filter(Boolean)
