...
import { runConfigCommand } from '../src/composition/config-root.js'
...
function harness({ state = STATE(), cwd = '/work/proj', readOnly = false } = {}) {
  const saves = []
  const stdout = []
  const stderr = []
  let current = state
  ...
  const uiCalls = []
  const run = (args, openUi = async (mode, opts) => {
