...
type HarnessOptions = { state?: State; cwd?: string; env?: EnvMap }

function harness({ state = STATE(), cwd = '/somewhere', env = {} }: HarnessOptions = {}) {
  const saves: State[] = []
  const store = {
 ...
 *
 * planLaunch returns `LaunchNeedsSetup | LaunchPlan`, discriminated on
 * `needsSetup`. Every case here seeds a state that HAS profiles, so every one
 * takes the LaunchPlan branch — there is no `needsSetup` assertion anywhere in
 * this file. Naming that once here is what keeps `r.args` and `r.plan` readable
 * at the ~30 call sites below instead of putting a cast on each. If a case ever
 * did come back needing setup, the field read is undefined and the assertion
 * fails exactly as it does today.
 */
function plan(argv: string[], opts: HarnessOptions = {}): LaunchPlan & { saves: State[] } {
...
test('an unknown --cc-provider lists the valid ids', () => {
  assert.throws(
    ...
    (err: Error) => /not a known provider/.test(err.message) && /openrouter/.test(err.message),
  )
})
