...
import { inspectAmbient, staleStoredModels } from '../../src/core/hygiene.js'
import { buildEnvPlan } from '../../src/core/env.js'
import { byId } from '../../src/adapters/providers/registry.js'
...
const codesOf = (ws) => ws.map((w) => w.code).sort()
const find = (ws, code) => ws.find((w) => w.code === code)
const planFor = (profile, provider, ambient) => buildEnvPlan(profile, provider, ambient)
...
test('a clean environment produces no warnings at all', () => {
  ...
  const plan = planFor({ apiKey: 'k' }, gateway, { PATH: '/usr/bin', HOME: '/home/u' })
...
test('an environment with unrelated ANTHROPIC-ish variables stays silent', () => {
  ...
  const plan = planFor({ apiKey: 'k' }, gateway, {
...
test('a stale ANTHROPIC_API_KEY produces a high-severity billing warning', () => {
  const plan = planFor({ apiKey: 'k' }, gateway, { ANTHROPIC_API_KEY: 'sk-ant-STALE' })
...
test('the billing warning does not leak the key itself', () => {
  ...
  const plan = planFor({ apiKey: 'k' }, gateway, { ANTHROPIC_API_KEY: secret })
...
test('first-party Anthropic replacing its own key warns differently', () => {
  ...
  const plan = planFor({ apiKey: 'profile-key' }, byId('anthropic'), {
...
test('an inherited base URL that gets overridden is reported with both values', () => {
  const plan = planFor({ apiKey: 'k' }, gateway, { ANTHROPIC_BASE_URL: 'https://stale.example' })
...
test('an inherited base URL that gets CLEARED is reported too', () => {
  ...
  const plan = planFor({ apiKey: 'k' }, byId('anthropic'), {
...
test('a tier pinned to a model without the wider window is flagged', () => {
  ...
  const plan = planFor(
    { apiKey: 'k', models: { opus: 'glm-4-air' } },
    byId('zai'),
    {},
  )
...
test('the fully-suffixed default z.ai profile triggers no tripwire', () => {
  const plan = planFor({ apiKey: 'k' }, byId('zai'), {})
...
test('an environment that disables 1M context is flagged loudly', () => {
  ...
  const plan = planFor({ apiKey: 'k' }, byId('zai'), { CLAUDE_CODE_DISABLE_1M_CONTEXT: '1' })
...
test('the 1M kill-switch is irrelevant for a provider without extended context', () => {
  const plan = planFor({ apiKey: 'k' }, gateway, { CLAUDE_CODE_DISABLE_1M_CONTEXT: '1' })
...
test('active compat flags are reported at info severity only', () => {
  ...
  const plan = planFor({ apiKey: 'k', compat: { disableAdaptiveThinking: true } }, gateway, {})
...
test('every warning carries a severity, a code and a message', () => {
  const plan = planFor({ apiKey: 'k', compat: { enableToolSearch: true } }, gateway, {
...
test('warnings are advisory: the plan is identical with and without a dirty env', () => {
  ...
  const clean = planFor({ apiKey: 'k' }, gateway, {})
  const dirty = planFor({ apiKey: 'k' }, gateway, {
    ANTHROPIC_BASE_URL: 'https://stale.example',
    ANTHROPIC_DEFAULT_OPUS_MODEL: 'stale',
  })
...
test('inspection reads the plan, never the whole environment', () => {
  ...
  const huge = { ANTHROPIC_API_KEY: 'sk-ant-STALE' }
...
test('staleStoredModels reports a suffix the provider cannot honour', () => {
  const found = staleStoredModels({ models: { opus: 'Pro/GLM[1m]' } }, byId('siliconflow'))
  assert.deepEqual(found, [
...
test('staleStoredModels is silent on a correct profile', () => {
  assert.deepEqual(staleStoredModels({ models: { opus: 'glm-5.2[1m]' } }, byId('zai')), [])
  assert.deepEqual(staleStoredModels({ models: {} }, byId('zai')), [])
  assert.deepEqual(staleStoredModels({}, byId('zai')), [])
})
