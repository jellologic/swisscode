...
import { SKIP_FLAG, buildArgs, parseArgv } from '../../src/core/args.js'
...
test('buildArgs prepends the skip flag according to the profile', () => {
  assert.deepEqual(buildArgs({ skipPermissions: true }, ['-p', 'x']), [SKIP_FLAG, '-p', 'x'])
  assert.deepEqual(buildArgs({ skipPermissions: false }, ['-p', 'x']), ['-p', 'x'])
})

test('a per-run override beats the stored preference', () => {
  assert.deepEqual(buildArgs({ skipPermissions: false }, [], true), [SKIP_FLAG])
  assert.deepEqual(buildArgs({ skipPermissions: true }, [], false), [])
})
...
test('the flag is never duplicated when the user typed it themselves', () => {
  ...
  assert.deepEqual(buildArgs({ skipPermissions: true }, [SKIP_FLAG]), [SKIP_FLAG])
  assert.deepEqual(buildArgs({ skipPermissions: true }, ['--', SKIP_FLAG]), ['--', SKIP_FLAG])
})

test('buildArgs copies rather than aliasing the passthrough array', () => {
  const passthrough = ['-p']
  const out = buildArgs({}, passthrough)
  out.push('mutated')
...
import { CC_FLAGS } from '../../src/core/args.js'
...
test('--cc-* flags never reach claude', () => {
  ...
  assert.ok(!buildArgs({}, r.passthrough).some((a) => a.startsWith('--cc-')))
})
...
test('a bare --cc-model sets all four tiers', () => {
  ...
  for (const t of TIERS) assert.equal(r.overrides.models[t], 'kimi-k3', t)
})
...
test('a model id containing = keeps everything after the first one', () => {
  assert.equal(parseArgv(['--cc-model', 'opus=vendor/model=v2']).overrides.models.opus, 'vendor/model=v2')
})
