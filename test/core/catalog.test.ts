...
import type { NormalizedModel } from '../../src/ports/catalog.ts'

/**
 * `over` is Record<string, unknown> and NOT Partial<NormalizedModel>, because
 * half these tests exist to prove isNormalizedModel REJECTS malformed rows —
 * `{tools: 'yes'}`, `{context: 'lots'}`, `{pricing: {prompt: 'free'}}`. Typing
 * the override as Partial would make those lines fail to compile, and the only
 * way to "fix" that would be to stop testing the validator.
 *
 * The result is asserted to be a NormalizedModel so it can still be handed to
 * rank/filterModels, which genuinely require one. The cast is the deliberate
 * boundary between "fixture the validator is meant to reject" and "row the
 * ranking code is allowed to assume".
 */
const model = (id: string, over: Record<string, unknown> = {}): NormalizedModel =>
  ({
    id,
    name: id,
    description: '',
