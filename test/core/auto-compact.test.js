...
import { autoCompactWindow, contextWindowFor } from '../../src/core/context.js'
...
test('contextWindowFor rejects nonsense captured windows', () => {
  for (const bad of [0, -1, NaN, Infinity, '900000', null]) {
    assert.equal(contextWindowFor('mystery', ec, { mystery: bad }), null, `accepted ${bad}`)
  }
})
...
test('the window is set alongside [1m], never instead of it', () => {
  ...
  const plan = buildEnvPlan({ apiKey: 'k' }, { ...gateway, extendedContext: ec }, {})
...
test('a catalog-captured window reaches the environment', () => {
  const plan = buildEnvPlan(
    { apiKey: 'k', models: { opus: 'gpt-x', sonnet: 'gpt-x', haiku: 'gpt-x', fable: 'gpt-x' }, contextWindows: { 'gpt-x': 128_000 } },
    gateway,
    {},
  )
...
test('first-party Anthropic never gets a window', () => {
  ...
  const plan = buildEnvPlan(
    { apiKey: 'k', models: { opus: 'claude-opus-4-8' }, contextWindows: { 'claude-opus-4-8': 200_000 } },
    firstParty,
    {},
  )
  assert.equal(plan.set[VAR], undefined)
