...
test('a malformed binding value resolves to nothing rather than throwing', () => {
  assert.equal(resolveBinding('/w/p', { '/w': 42 }), null)
  assert.equal(resolveBinding('/w/p', { '/w': {} }), null)
})
...
test('binding refuses an unknown profile or a relative path', () => {
  assert.match(bindPath(state(), '/work/a', 'nope').reason, /not a profile/)
  assert.match(bindPath(state(), 'relative', 'z').reason, /not an absolute path/)
})
...
test('prune drops dead directories and dangling profiles, and nothing else', () => {
  const alive = (key) => key !== '/work/a/b/c'
