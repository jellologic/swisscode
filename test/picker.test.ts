...
const msModel = (id: string, tools: boolean | null = null): NormalizedModel => ({
  id,
  name: id,
  description: '',
  context: null,
...
// The four casts below, and why they are not a loosening.
//
// `assert.equal(result, undefined)` further up is an ASSERTION SIGNATURE in
// @types/node (`asserts actual is T`), so from that line on TypeScript believes
// `result` is permanently `undefined` — and then `assert.ok(result)` narrows it
// to `never`. It cannot see the `onResult` callback assigning it during the
// keystrokes in between. The test is right and the narrowing is stale.
//
// Re-widened at each read rather than by introducing a local, so the statements
