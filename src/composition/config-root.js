...
// (core/migrate.js SOFT_RESERVED), so the second token can never be ambiguous.
//
...
import { validateProfileName } from '../core/migrate.js'
...
async function openWizard({ deps, openUi, name, mode = 'config', err, out }) {
...
function listProfiles({ deps, out }) {
...
function credentialOrigin(profile) {
...
function setDefault({ deps, name, out, err }) {
...
function removeProfile({ deps, name, out, err }) {
  const { state, readOnly } = deps.store.load()
  if (readOnly) return refuseWrite(err)
  if (!name) {
...
function useCommand({ deps, head, args, out, err }) {
  const loaded = deps.store.load()
 ...
 */
function showBinding({ deps, loaded, cwd, out }) {
...
function unbindCommand({ deps, path, out, err }) {
  const loaded = deps.store.load()
...
function listBindings({ deps, prune, out, err }) {
  const loaded = deps.store.load()
...
async function doctorCommand({ deps, args, out, err }) {
  ...
  const { runDoctor } = await import('./doctor-root.js')
