...
import { TIER_ENV_VARS } from './tiers.js'
import { SUFFIX, bareModelId, supportsExtendedContext } from './context.js'

/** Highest-cost failure mode in the tool: it bills someone else's account. */
export const BILLING_KEY = 'ANTHROPIC_API_KEY'

const w = (severity, code, message) => ({ severity, code, message })

/**
 * @param {{set:Record<string,string>, unset:string[]}} plan
 * @param {Record<string,string>} ambientEnv
 * @param {object} ctx
 * @returns {Array<{severity:string, code:string, message:string}>}
 */
export function inspectAmbient(plan, ambientEnv = {}, ctx = {}) {
  const warnings = []
  const set = plan?.set ?? {}
 ...
 */
export function staleStoredModels(profile, provider) {
