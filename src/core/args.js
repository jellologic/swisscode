...
//                       core/migrate.js, so it can only match a name someone
//                       deliberately made
//
 ...
 * @property {string|null} command        'config' | 'setup'
 * @property {string[]} commandArgs
 * @property {string[]} passthrough       --cc-* already stripped; claude never sees them
 * @property {boolean|null} skipOverride
 * @property {string|null} profileFlag    --cc-profile NAME
 * @property {string|null} positional     argv[0], if it could name a profile
 * @property {{provider?:string, baseUrl?:string, models?:object, env?:object}} overrides
 * @property {string|null} error          set => exit 2 with this message
 */
...
export function parseArgv(argv = []) {
  ...
  if (argv.length > 0 && CONFIG_COMMANDS.includes(argv[0])) {
    return { ...base, command: argv[0], commandArgs: argv.slice(1) }
  }
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (name === '--cc-model') {
      ...
      if (split === -1) {
        ...
        models = Object.fromEntries(TIERS.map((t) => [t, value]))
        continue
      }
 ...
 * selecting accounts.
 */
function profileCandidate(first) {
 ...
 */
export function buildArgs(profile, passthrough = [], skipOverride = null) {
