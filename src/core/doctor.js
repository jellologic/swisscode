...
import { SUFFIX, bareModelId } from './context.js'
import { staleStoredModels } from './hygiene.js'
import { SOFT_RESERVED } from './migrate.js'
...
export function makeCheck(id, title, status, detail, extra = {}) {
 ...
 *
 * @param {object} input
 * @param {object} input.loaded        store.load() result
 * @param {object} input.selection     resolveProfile() result
 * @param {object|null} input.profile  after overrides
 * @param {object|null} input.provider descriptor, or null when unknown
 * @param {object|null} input.plan     buildEnvPlan() result
 * @param {{dir:number|null, file:number|null}} input.modes
 * @param {{path:string|null, error:string|null}} input.binary
 * @param {string|null} input.cwd
 * @param {string[]} input.deadBindingPaths  supplied by the adapter; [] if unchecked
 */
export function staticChecks(input) {
  const {
    loaded,
    selection,
    profile,
  ...
  // ---- models --------------------------------------------------------------
  const resolved = plan?.resolvedModels ?? {}
...
function modeCheck(id, title, mode, want) {
 ...
 */
export function probeSpec(profile, provider, plan) {
  const baseUrl = plan?.set?.ANTHROPIC_BASE_URL ?? null
  const credentialEnv = provider?.credentialEnv ?? 'ANTHROPIC_AUTH_TOKEN'
  const credential = plan?.set?.[credentialEnv] ?? null
 ...
 */
export function interpretMessagesProbe({ model, result, provider }) {
...
export function interpretToolProbe({ model, result }) {
...
export function remainingBudget(startedAt, now, totalMs, perProbeMs) {
